...
interface WalletContextType extends WalletState {
  ...
  sendTransaction: (toAddress: string, amountSats: number, feeRate: number, opReturn?: string, opReturnIsHex?: boolean) => Promise<string>;
...
export function WalletProvider({ children }: { children: React.ReactNode }) {
  ...
  const sendTransaction = useCallback(
    async (toAddress: string, amountSats: number, feeRate: number, opReturn?: string, opReturnIsHex?: boolean): Promise<string> => {
      ...
      let txHex: string;
      try {
        txHex = buildAndSignTx(wif, inputs, [{ address: toAddress, value: amountSats }], feeRate, opReturn, opReturnIsHex);
      } catch (buildErr) {
